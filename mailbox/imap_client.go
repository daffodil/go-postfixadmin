...
func CreateImapClient(resp http.ResponseWriter, req *http.Request)(*imap.Client) {
	...
	if addr_err != nil {
		base.WriteErrorJSON(resp, "Invalid email address")
		return nil
	}
	...
	if err != nil {
		base.WriteErrorJSON(resp, err.Error())
		return nil
	}

	//= Connect to Server
	client, conn_err := imap.DialTLS(conf.IMAPServer.Server, tlsConfig)
	if conn_err != nil {
		base.WriteErrorJSON(resp, conn_err.Error())
		return nil
	}
	_, login_err := client.Login( addr.Address, pass )
	if login_err != nil {
		base.WriteErrorJSON(resp, "IMAP login error")
		return nil
	}
	return client
